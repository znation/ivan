...
template <class type> struct fearray
{
  ...
  fearray(const fearray<type>& A) : Data(A.Data), Size(A.Size) {
    if (Data) {
      ++REFS(Data);
    }
  }
  fearray(const type* Array, sizetype Size) : Size(Size) {
  ...
  fearray& operator=(const fearray& A) {
    if(Data)
      ++REFS(Data = A.Data);
