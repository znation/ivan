...
mkdir -p "${GAME_DIR}"
cp -R "${SCRIPT_DIR}/VALE.app" "${GAME_DIR}"

DATA_DIR="${GAME_DIR}/VALE.app/Contents"

mkdir -p "${DATA_DIR}"/{MacOS,Resources,Frameworks}
cp "${BUILD_DIR}"/bin/vale "${DATA_DIR}/MacOS"
cp -R "${BUILD_DIR}"/docs/* "${GAME_DIR}"
cp -R "${BUILD_DIR}"/vale "${DATA_DIR}/Resources/data"
cp -R "${SDL2DIR}"/*.framework "${DATA_DIR}/Frameworks"
ln -s /Applications "${GAME_DIR}/Applications"
cat > "${GAME_DIR}/INSTALL.txt" << 'EOT'
To install, simply drag VALE.app into the Applications folder.
...
"/Users/USERNAME/Library/Application Support/VALE".
EOT

BIN="${DATA_DIR}/MacOS/vale"
